...
public class DeviceInfo {
    ...
    public static long getAvailMemory(Context c) {
        long availMem = DEVICEINFO_UNKNOWN;
    ...
    public static String getCpuInfo() {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (TextUtils.equals(entry[0].toLowerCase().trim(), "hardware"))
                    return entry[1].trim();
            }
        } catch (IOException e) {
            e.printStackTrace();
        ...
        //如果在cpuinfo找不到
        return String.valueOf(Build.HARDWARE).trim();
    }
...
//        }
        arch  = Build.CPU_ABI;
        Log.d("getCpuArchType", "arch "+arch);
        return arch;
    }
}