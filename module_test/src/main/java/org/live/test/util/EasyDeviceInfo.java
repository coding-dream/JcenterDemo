...
public class EasyDeviceInfo {
    ...
    private static final String initialVal = "";
...
//            result = metrics.heightPixels + "x" + metrics.widthPixels;
    ...
    public String getCarrier() {
        ...
        try {
            if (tm != null && tm.getPhoneType() != TelephonyManager.PHONE_TYPE_CDMA) {
                result = tm.getNetworkOperatorName().toLowerCase(Locale.US);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            for (NetworkInterface nif : all) {
                ...
                for (byte macByte : macBytes) {
                    sb.append(String.format(Locale.US, "%02x", macByte & 0xFF)).append(":");
                }
                if (sb.length() > 0) {
    ...
    public String getPsuedoUniqueID() {
        ...
        devIDShort += (Build.DEVICE.length() % 10)
...
//    }
    ...
    public String getAvailMemory() {
        long avaMem = DeviceInfo.getAvailMemory(context);
        return Formatter.formatFileSize(context, avaMem);// 将获取的内存大小规格化
    }
    ...
    public String getTotalMemory() {
        long avaMem = DeviceInfo.getTotalMemory(context);
        return Formatter.formatFileSize(context, avaMem);
    }
}
